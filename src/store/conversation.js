...
// import { useUser } from "./useMessengerStore";
// const { id } = useUser();
  ...
  const newConversation = async (ConversationId, senderId, receiverId) => {
    try {
      ...
      const foundConversation =
        conversationStore.getConversation.conversations.find((conversation) =>
      ...
      const newConversation = await apiClient.post("/newConversation", {
        ConversationId,
        senderId,
        receiverId,
      });
  ...
  const getFriendConversation = async () => {
    try {
      const response = await apiClient(
        "/getFriendConversation/" + conversationStore.getFreindId
      );

      const conversation = response.data[0].conversations.find((conversation) =>
        conversation.members.some((m) => m === messengerStore.userStore.id)
      );
