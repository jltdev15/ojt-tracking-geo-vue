<template>
  <div class="p-3">
    ...
    <section class="flex flex-col p-3 md:flex-row gap-9">
      <div class="p-3 rounded-lg shadow-md md:w-3/6">
        ...
        <div class="py-3">
          <ul>
            <p class="font-medium text-center" v-if="adminUserStore.departmentlist.length === 0">
              No data found
            </p>
            <li v-else v-for="(item, index) in adminUserStore.departmentlist" :key="index"
              class="flex items-center justify-between gap-3 p-2 bg-gray-50">
              <div class="w-full">
                <input :disabled="updateEnabled !== index" type="text" class="w-full p-3" v-model="item.name" />
              </div>

              <div class="flex items-center gap-3">
                <span @click="toggleEdit(index, item.name, item._id)"
                  class="p-2 cursor-pointer bg-primary text-gray-50">
                  {{ updateEnabled === index ? "Save" : "Edit" }}</span>

                <span @click="adminUserStore.deleteDepartmentList(item._id)"
                  class="p-2 bg-red-600 cursor-pointer text-gray-50">Remove</span>
              </div>
            </li>
          </ul>
        </div>
      ...
      <div class="p-3 rounded-lg shadow-md md:w-3/6">
        ...
        <div class="flex justify-end gap-3">
          <input type="text" placeholder="Search Username" class="w-full input" v-model="searchValue" />
        ...
        <div class="py-3">
          <EasyDataTable :headers="headers" :items="adminUserStore.getInternAccounts" :search-field="searchField"
            ...
            <template #item-operation="item">
              <div>
                <button @click="toggleConfirmationModal(item.profile._id)" class="btn btn-sm btn-accent">
                  Reset Account
                </button>
              </div>
            </template>
