    ...
    <div class="py-2 px-6 bg-[#f8f4f3] flex items-center shadow-md shadow-black/5 sticky top-0 left-0 z-30">
      ...
      <ul class="flex items-center ml-auto">
        <li class="ml-3 dropdown">
          <button @click="handleToggleDropDown" type="button" class="flex items-center dropdown-toggle">
            <div class="relative flex-shrink-0 w-10 h-10">
              <div class="p-1 bg-white rounded-full focus:outline-none focus:ring">
                <img class="w-8 h-8 rounded-full"
                  src="https://laravelui.spruko.com/tailwind/ynex/build/assets/images/faces/9.jpg" alt="" />
                <div class="absolute top-0 w-3 h-3 border-2 border-white rounded-full left-7 bg-lime-400 animate-ping">
                </div>
              ...
              <h2 class="text-sm font-semibold text-gray-800 capitalize ">
                {{
                  authStore.currentUser
                    ? authStore.currentUser
                    : authStore.internInformation.userName
                }}
              </h2>
      ...
      <ul :class="{ hidden: !isDropDownShow }"
        ...
        <li>
          <router-link :to="{ name: 'intern_profile' }"
            class="flex items-center text-[13px] py-1.5 px-4 text-gray-600 hover:text-[#f84525] hover:bg-gray-50">Profile</router-link>
        </li>
        <li>
          <router-link :to="{ name: 'intern_settings' }"
            class="flex items-center text-[13px] py-1.5 px-4 text-gray-600 hover:text-[#f84525] hover:bg-gray-50">Settings</router-link>
        </li>
        <li>
